export const navLinks = [
    {
        name: "home",
        route: "/",
        icon: "images/home.png",
    },
    {
        name: "projects",
        route: "/projects",
        icon: "images/projects.png",
    },
    {
        name: "about",
        route: "/about",
        icon: "images/about.png",
    },
    {
        name: "contact",
        route: "/contact",
        icon: "images/contact.png",
    },
];

export const skills = [
    { name: "git", image: "/skills/git.png" },
    { name: "github", image: "/skills/github.png" },
    { name: "mongodb", image: "/skills/mongodb.png" },
    { name: "sql", image: "/skills/sql.png" },
    { name: "postgresql", image: "/skills/pg.png" },
    { name: "express", image: "/skills/express.png" },
    { name: "nodejs", image: "/skills/nodejs.png" },
    { name: "reactjs", image: "/skills/reactjs.png" },
    { name: "nextjs", image: "/skills/nextjs.png" },
    { name: "redux", image: "/skills/redux.png" },
    { name: "html", image: "/skills/html.png" },
    { name: "css", image: "/skills/css.png" },
    { name: "ejs", image: "/skills/ejs.png" },
    { name: "tailwind", image: "/skills/tailwind.png" },
    { name: "bootstrap", image: "/skills/bootstrap.png" },
    { name: "java", image: "/skills/java.png" },
    { name: "c++", image: "/skills/cpp.png" },
    { name: "c", image: "/skills/c.png" },
    { name: "javascript", image: "/skills/js.png" },
    { name: "typescript", image: "/skills/ts.png" },
    { name: "docker", image: "/skills/docker.png" },
    { name: "aws", image: "/skills/aws.png" },
    { name: "nginx", image: "/skills/nginx.png" },
    { name: "socket.io", image: "/skills/socketio.png" },
];

export const projects = [
    {
        name: "CoDocs",
        desc: "A real-time collaborative document editor with WebSocket-based synchronization.",
        image: "https://via.placeholder.com/300x200?text=CoDocs",
        repoLink: "https://github.com/adiv933/CoDocs",
        liveLink: "",
        skills: ["React", "Typescript", "Node.js", "MongoDB", "Socket.IO"],
    },
    {
        name: "FoodOS",
        desc: "A full-stack food ordering platform built with React, Node.js, and SQL.",
        image: "https://via.placeholder.com/300x200?text=FoodOS",
        repoLink: "https://github.com/adiv933/FoodOS",
        liveLink: "https://food-os.vercel.app",
        skills: ["React", "Node.js", "Express", "SQL Plus", "Tailwind CSS", "Express"],
    },
    {
        name: "TopFlow",
        desc: "A modern team collaboration tool featuring task management and real-time updates.",
        image: "https://via.placeholder.com/300x200?text=TopFlow",
        repoLink: "https://github.com/adiv933/TopFlow",
        liveLink: "https://top-flow.vercel.app",
        skills: ["React", "Node.js", "MongoDB", "Socket.IO", "Tailwind CSS", "Express"],
    },
    {
        name: "Lost&Found",
        desc: "An Android app designed to help users find and track lost items efficiently.",
        image: "https://via.placeholder.com/300x200?text=Lost%26Found",
        repoLink: "https://github.com/adiv933/LostAndFound",
        liveLink: "https://play.google.com/store/apps/details?id=com.aditya.lostandfound",
        skills: ["Java", "Android Studio", "Firebase", "Google Maps API"],
    },
];

